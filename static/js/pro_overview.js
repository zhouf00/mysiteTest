            ...
            success: function (msg) {
                ...
                if(msg.data){
                    for(var i=0; i<msg.data.length;i++){
                        result.push(
                        {
                            facName:msg.data[i]['items__name'],
                            name:msg.data[i]['name'],
                            address:msg.data[i]['items__name'],
                            x:msg.data[i]['items__x'],
                            y:msg.data[i]['items__y'],
                            status:msg.data[i]['status']
                        }
                    )
                    }
                }
                ...
                if (null != result && result.length > 0) {
                    ...
                    function buildData(obj) {
                        if (provinces[obj.address] != null) {
                            ...
                            if (dataArr[province] == null) {
                                ...
                                dataArr[province] = dataArr[province].concat([{
                                    "name": obj.name,
                                    "facName": obj.facName,
                                    "value": [obj.loc_x, obj.loc_y, obj],
                    ...
                    for (var i = 0; i < result.length; i++) {
                        ...
                        styleArr.push(obj);
                        locArr = locArr.concat([{
                            "name": result[i]["name"],
                            "value": [result[i]["x"], result[i]["y"], obj],
                            "itemStyle": obj.itemStyle
                        }]);
                        buildProvince(obj);
                        buildData(obj);
