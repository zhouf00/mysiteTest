...
class RoleMenuView(LoginRequiredMixin, View):
    """
    角色绑定菜单
    """
    def get(self, request):
...
class RoleMenuListView(LoginRequiredMixin, View):
    """
    获取菜单列表
    """
    def get(self, request):
...
class RoleUserView(LoginRequiredMixin, View):
    """
    角色关联用户
    """
    def get(self, request):
    ...
    def post(self, request):
        ...
        if id_list:
            for user in User.objectsd.filter(id__in=id_list):
